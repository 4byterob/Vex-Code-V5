...
void autonomous(void) {
  ...
  All.spinFor(vex::forward, 1 * mult, turns);

  motor_group Arms = motor_group(L_ArmSpin, R_ArmSpin);
  Arms.spinFor(vex::forward, 0.1, turns);
  Arms.spinFor(vex::reverse, 0.1, turns);
  Arms.spinFor(vex::forward, 0.1, turns);
  Arms.spinFor(vex::reverse, 0.1, turns);
  Arms.spinFor(vex::forward, 0.1, turns);
  Arms.spinFor(vex::reverse, 0.1, turns);

  ClawSpin.spinFor(vex::forward, 0.5, turns);

  All.spinFor(vex::reverse, 1 * mult, turns);
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if (!gameMode){
      ...
      Controller1.ButtonL2.pressed(FlipArms);
      Controller1.ButtonL1.pressed(FlipClaw);
